...
contract SupplyChain {
  ...
  modifier harvested(uint _upc) {
    require(items[_upc].itemState == State.Harvested, "Item is not in the the harvested state");
    _;
  }
  ...
  modifier packed(uint _upc) {
    require(items[_upc].itemState == State.Packed, "Item is not in the the packed state");
    _;
  }
  ...
  function receiveItem(uint _upc) public 
    // Call modifier to check if upc has passed previous supply chain stage
    //shipped(_upc)
    // Access Control List enforced by calling Smart Contract /h DApp
    {
